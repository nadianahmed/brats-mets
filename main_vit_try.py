...
class T1cMRI_Dataset(Dataset):
    def __init__(self, dataframe, transform=None, use_thresholding=False, use_template_matching=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_template_matching:
            t1c_image = self.apply_template_matching(t1c_image)

        # Convert to PyTorch tensor (2 channels: MRI + Thresholded/Template Image)
        t1c_image = torch.tensor(t1c_image, dtype=torch.float32).unsqueeze(0)  # (1, D, H, W)
    ...
    def apply_template_matching(self, image):
        from scipy.ndimage import gaussian_filter, maximum_filter, label, center_of_mass
        from skimage.feature import match_template

        radius = 10  # Adjustable
        template = self.create_spherical_template(radius)
        correlation = match_template(image, template, pad_input=True)
        threshold = 0.6 * np.max(correlation)
        detected_peaks = (correlation > threshold)

        masked = np.zeros_like(image)
        masked[detected_peaks] = image[detected_peaks]

        return np.stack([image, masked])  # (2, D, H, W)

    def create_spherical_template(self, radius):
        shape = (2 * radius + 1,) * 3
...
class VisionTransformer3D(nn.Module):
    def __init__(self, img_size=(240, 240, 155), patch_size=(16, 16, 16), in_channels=1, num_classes=4, embed_dim=768, num_heads=12, depth=12):
        ...
        self.fc = nn.Conv3d(embed_dim, 4, kernel_size=1)  # 1x1 Conv for 4-class voxel-wise segmentation
